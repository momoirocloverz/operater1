    ...
    <div class="dataListCon" v-loading="loading" element-loading-text="正在请求中...">
        <div>
            ...
            <div class="pt30">
                ...
                <div v-show="showNull"
                     style="width:100%; height:450px;background:#ffffff;text-align:center;padding-top:160px">暂无数据
                </div>
                <div v-show="!showNull" id="chartBus" style="width:100%; height:450px;background:#ffffff;"></div>
                <div class="pt30 pb20"></div>
                <el-row class="flex-box flex-right pr30 pt30 mt20 pos-r">
                    <el-button type="primary" plain v-for="(item, index) in btnUserList"
                               :class="{'btn-active':index === activeUserIndex}"
                               @click="searchData(item,index,2)"
                               :key="index">{{item.name}}
                    </el-button>
                    <el-date-picker
                            v-model="valueUserDate"
                            type="daterange"
                            ref="dateUserPicker"
                            class="dete-one-box"
                            value-format="yyyy-MM-dd" format="yyyy-MM-dd"
                            @change="handleChangeDateUser"
                            range-separator="至"
                            start-placeholder="开始日期"
                            end-placeholder="结束日期">
                    </el-date-picker>
                ...
                <div v-show="!showNullUser" id="chartUser" style="width:100%; height:450px;background:#ffffff;"></div>
            </div>
        </div>
    </div>
</template>
